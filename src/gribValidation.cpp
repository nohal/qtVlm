...
gribValidation::gribValidation(myCentralWidget * my_centralWidget,MainWindow * mainWindow) :  QDialog(my_centralWidget)
{
    setupUi(this);
...
gribValidation::~gribValidation()
{

}

void gribValidation::setMode(int mode)
{
    type->setCurrentIndex(mode-1);
    curMode=mode;
}

void gribValidation::done(int result)
{
    int newMode;
...
void gribValidation::doNow(void)
{
    tstamp->setText(QString().setNum(QDateTime::currentDateTime().toUTC().toTime_t()));
}

void gribValidation::interpolationChanged(int newMode)
{
    if(this->my_centralWidget->getGrib())
    {
        newMode++;
...
void gribValidation::inputChanged(void)
{
