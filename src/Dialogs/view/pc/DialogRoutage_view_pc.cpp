...
DialogRoutage_view_pc::DialogRoutage_view_pc(myCentralWidget *centralWidget,DialogRoutage_ctrl * ctrl):
...
void DialogRoutage_view_pc::initDialogState(RoutageData * routageData) {
    QString m;
    ...
    inputTraceColor =new InputLineParams(routageData->lineWidth,routageData->lineColor,1.6,  QColor(Qt::red),this,0.1,5);
...
void DialogRoutage_view_pc::set_dialogVisibility(bool visible) {
    if(visible) {
        show();
...
void DialogRoutage_view_pc::closeEvent(QCloseEvent * ) {
    slot_cancel();
}

void DialogRoutage_view_pc::slot_ok() {
...
void DialogRoutage_view_pc::slot_cancel() {
    qWarning() << "Slot cancel";
    Settings::saveGeometry(this);
    ctrl->exitDialog();
}

void DialogRoutage_view_pc::slot_default()
{
...
void DialogRoutage_view_pc::GybeTack(int i)
{
